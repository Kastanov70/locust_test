...
class QuickstartUser(HttpUser):
    ...
    def from_other_operators(self):
        ...
        with self.client.get("/kafka", catch_response=True, headers=headers,
    ...
    def from_tele2_old(self):
        ...
        with self.client.get("/kafka", catch_response=True, headers=headers, name="/from_tele2_old") as response:
            if response.status_code != 200:
                response.failure("Request failure!")
            elif response.cookies.keys()[0] != 'cookieparameters':
                response.failure("No 'cookieparameters' received!")
    ...
    def from_tele2_new(self):
        ...
        headers = {"Host": "clid.tele2.ru", "X-Request-ID": "8098892511a4c0c7ceb734539a27862a",
                   ...
                   "X-MSISDN": ('88' + str(random.randint(100000000, 999999999))),
                   "X-IMSI": str(random.randint(100000000000000, 999999999999999))}

        with self.client.get("/kafka", catch_response=True, headers=headers, name="/from_tele2_new") as response:
            if response.status_code != 200:
                response.failure("Request failure!")
            elif response.cookies.keys()[0] != 'cookieparameters':
                response.failure("No 'cookieparameters' received!")
